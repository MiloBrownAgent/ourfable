-- Skip profiles (already created), create everything else
...
create table if not exists public.books (
  ...
  art_style text not null default 'watercolor' check (art_style in ('watercolor', 'whimsical', 'soft_pastel', 'bold_pop', 'fantasy', 'classic')),
...
create table if not exists public.orders (
  ...
  status text not null default 'pending' check (status in ('pending', 'paid', 'processing', 'shipped', 'delivered', 'refunded')),
...
insert into storage.buckets (id, name, public) values ('character-photos', 'character-photos', false) on conflict do nothing;
insert into storage.buckets (id, name, public) values ('book-assets', 'book-assets', true) on conflict do nothing;

create policy "Users can upload character photos" on storage.objects for insert with check (bucket_id = 'character-photos' and auth.uid()::text = (storage.foldername(name))[1]);
create policy "Users can view own character photos" on storage.objects for select using (bucket_id = 'character-photos' and auth.uid()::text = (storage.foldername(name))[1]);
