...
function purchaserHtml({ purchaserName, recipientName, giftCode }: { purchaserName: string; recipientName?: string; giftCode: string }) {
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;">
        ...
        <tr><td style="background:#FFFFFF;border-radius:20px;border:1px solid #EAE7E1;overflow:hidden;">
          ...
          <table width="100%"><tr><td style="padding:44px;">
            ...
            <p style="margin:0 0 28px;font-family:Georgia,serif;font-size:26px;color:#1A1A1A;line-height:1.3;">Hi ${purchaserName} —</p>
            <p style="margin:0 0 20px;font-family:-apple-system,sans-serif;font-size:15px;color:#4A4A4A;line-height:1.8;">You've given ${recipient} something that will last 18 years. Their gift code is below.</p>
            <div style="background:#F8F5F0;border:1px solid #E0DDD7;border-radius:12px;padding:20px 24px;text-align:center;margin:0 0 28px;">
              ...
              <p style="margin:0;font-family:Georgia,serif;font-size:32px;font-weight:400;color:#4A5E4C;letter-spacing:0.08em;">${giftCode}</p>
            </div>
            <p style="margin:0 0 28px;font-family:-apple-system,sans-serif;font-size:14px;color:#6A6660;line-height:1.7;">Share this code with ${recipient}, or send them this link:</p>
...
function recipientHtml({ purchaserName, recipientName, message, giftCode }: { purchaserName: string; recipientName?: string; message?: string; giftCode: string }) {
  const redeemUrl = `https://ourfable.ai/redeem/${giftCode}`;
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;">
        ...
        <tr><td style="background:#FFFFFF;border-radius:20px;border:1px solid #EAE7E1;overflow:hidden;">
          <table width="100%"><tr><td style="background:#4A5E4C;height:3px;font-size:0;">&nbsp;</td></tr></table>
          <table width="100%"><tr><td style="padding:44px;">
            <p style="margin:0 0 8px;font-family:-apple-system,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#8A9E8C;">A gift from ${purchaserName}</p>
            <p style="margin:0 0 24px;font-family:Georgia,serif;font-size:26px;color:#1A1A1A;line-height:1.3;">Hi ${rFirst} —</p>
            <p style="margin:0 0 20px;font-family:-apple-system,sans-serif;font-size:15px;color:#4A4A4A;line-height:1.8;">${purchaserName} gave your child Our Fable — a private memory vault that interviews the people in your child's life, every month, and holds every answer until your child is ready.</p>
            ${message ? `<div style="border-left:3px solid #C8D4C9;padding:12px 16px;margin:0 0 24px;"><p style="margin:0;font-family:Georgia,serif;font-size:15px;color:#4A4A4A;line-height:1.75;font-style:italic;">"${message}"</p></div>` : ""}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await sendEmail({
      ...
      html: purchaserHtml({ purchaserName, recipientName, giftCode }),
    ...
    if (recipientEmail) {
      await sendEmail({
        ...
        html: recipientHtml({ purchaserName, recipientName, message, giftCode }),
        headers: unsubHeaders,
      });
    }
